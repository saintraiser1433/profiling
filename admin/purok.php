...
if (isset($_POST['submit'])) {
  $name = $_POST['puroks'];
  $querys = "SELECT * from purok where purok='$name'";
  $stmts = $conn->prepare($querys);
  $stmts->execute();
  $results = $stmts->get_result();
  $row = $results->fetch_assoc();
  if ($name == $row['purok']) {
    $_SESSION['response'] = "This purok is already exist";
    $_SESSION['type'] = "danger";
  } else {
    $sql = "INSERT into purok (purok) values (?)";
...
if (isset($_POST['update'])) {
  $name = $_POST['puroks1'];
  $id = $_POST['puroks2'];
  $sql = "UPDATE purok SET purok=? where id=?";
  $stmt = $conn->prepare($sql);
